        ...
        <div class="header-body row">
            ...
            @if($income_messages)
                ...
                <div id="display" class="col-lg-3" style="display: inline;background-color: #fff; padding: 20px 10px; border-radius: 5px;   margin-right: 5px;margin-left: 15px;margin-bottom: 10px;">
                 ...
                 <a  href=""  class="{{$income_message->message_to}}" > {{$income_message->message_to}} </a>

                  <i class="fab fa-facebook-messenger" style="color: #f4645f;">{{$income_message->count}}</i>
                  <div class='prev {{$income_message->message_to}}' style="display:none; border-top-left-radius:9px;border-top-right-radius:9px;  position: fixed; bottom: 2px; background: rgba(84, 99, 121, 0.5);width: 279px; min-height: 300px; margin-left: 17px;">
                <span   style="border-top-left-radius:7px;border-top-right-radius:7px;width: 100%; background-color: #232b58d9; display: inline-block;padding-left: 7px;font-weight: bold;color: #fff; margin-bottom: 7px;">{{$income_message->message_to}}</span>
                <form class="f" method="post" action="{{ route('messages.newmessage') }}" autocomplete="off">
                            @csrf 
                            @method('put')
           ...
           // console.log(results);
            ...
            $('.message').remove();
              ...
              results.forEach(function(item) {
                  var $sent_to = 'div.'+item["message_to"] +" " +".f";
                var $data =  $($sent_to);
                //console.log($sent_to);
